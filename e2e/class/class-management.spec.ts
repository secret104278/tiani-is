...
test.describe("YiDeClass Management", () => {
  ...
  test("should have correct admin links on detail page", async ({
    ...
    const unitSlug = "yide";
    await page.goto(
      `/class/${unitSlug}/activity/detail/${publishedClassActivity.id}`,
    );
    ...
    await expect(checkRecordLink).toHaveAttribute(
      "href",
      `/class/${unitSlug}/activity/checkrecord/${publishedClassActivity.id}`,
    );

    const leaveRecordLink = page.getByRole("link", { name: "請假名單" });
    await expect(leaveRecordLink).toHaveAttribute(
      "href",
      `/class/${unitSlug}/activity/leaverecord/${publishedClassActivity.id}`,
    );

    const absentLink = page.getByRole("link", { name: "缺席名單" });
    await expect(absentLink).toHaveAttribute(
      "href",
      `/class/${unitSlug}/activity/absent/${publishedClassActivity.id}`,
    );
  });
});
