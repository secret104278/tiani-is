...
test.describe('4. Profile Management', () => {
  test('should update user profile Tao information', async ({ page }) => {
    ...
    await page.getByRole('row', { name: 'E2E Test User' }).getByRole('button').click();
    ...
    await expect(page.locator('input[name="qiudaoDateSolar"]')).toHaveValue('2023-01-01');
    await expect(page.getByRole('combobox')).toHaveValue('子');
    await expect(page.locator('input[name="qiudaoTemple"]')).toHaveValue('Test Temple');
    await expect(page.locator('input[name="qiudaoTanzhu"]')).toHaveValue('Test Host');
    await expect(page.locator('input[name="affiliation"]')).toHaveValue('Test Unit');
    await expect(page.locator('input[name="dianChuanShi"]')).toHaveValue('Test Transmitter');
    await expect(page.locator('input[name="yinShi"]')).toHaveValue('Test Introducer');
    await expect(page.locator('input[name="baoShi"]')).toHaveValue('Test Guarantor');
  });
});
